...
# Read Input Image
img_ori = cv2.imread('4.jpg', cv2.IMREAD_GRAYSCALE)

height, width = img_ori.shape

# Maximize Contrast (Optional)
structuringElement = cv2.getStructuringElement(cv2.MORPH_RECT, (3, 3))
imgTopHat = cv2.morphologyEx(img_ori, cv2.MORPH_TOPHAT, structuringElement)
imgBlackHat = cv2.morphologyEx(img_ori, cv2.MORPH_BLACKHAT, structuringElement)
imgGrayscalePlusTopHat = cv2.add(img_ori, imgTopHat)
...
for i, matched_chars in enumerate(matched_result):
    ...
    img_cropped = cv2.getRectSubPix(img_rotated, patchSize=(int(plate_width), int(plate_height)),
...
for i, plate_img in enumerate(plate_imgs):
    ...
    plate_img_gray = plate_img  # Use the same grayscale image
